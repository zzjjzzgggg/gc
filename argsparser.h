...
public:
    ArgsParser(const int args, char** argv):
        num_args_(args), verbose_(false) {
        for (int i=0; i<args; i++) args_.push_back(argv[i]);
        if(args == 2 && (args_[1] == "-h" || args_[1] == "--help"))
            verbose_ = true;
        if(verbose_) printf("usage: %s\n", GetCmd().c_str());
    }
    virtual ~ArgsParser() {}

    const string GetCmd() const;

    const int GetIntArg(const string& arg, const int default_val,
                        const string& help) const;

    const double GetFltArg(const string& arg, const double default_val,
                           const string& help) const;

    const string GetStrArg(const string& arg, const string& default_val,
                           const string& help) const;

    // don't worry about returning a     std::vector, as a move operation will be
    // used under c++11.
    std::vector<int>
    GetIntVecArg(const string& arg,
                 const std::vector<int>& default_val,
                 const string& help) const;

    std::vector<string>
    GetStrVecArg(const string& arg,
